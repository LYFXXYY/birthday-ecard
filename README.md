...
- **模板管理** — 贺卡 HTML 模板编辑、预览、适用等级配置（employee_level）、页数设置（page_count）、祝福语绑定
- **祝福语管理** — 祝福语增删改查
- **发送记录** — 历史记录查询、短信内容展示、手机模拟器预览、重发
- **定时任务** — 每日 08:00 自动检查生日并发送祝福（防重复发送）
- **贺卡访问** — 独立 HTML 页面，4 页（员工）/ 7 页（管理层）翻页式设计，适配移动端浏览器
- **视频录制** — Playwright + FFmpeg 自动将贺卡录制为 MP4 视频（≤2MB），随彩信发送
- **系统监控** — 独立监控进程，实时检测 HTTP/端口/心跳/数据库状态，异常自动告警
- **操作日志** — 管理员所有 CRUD 操作自动记录，支持查询和统计

## 技术栈

| 层次 | 技术 | 说明 |
|------|------|------|
| 后端 | Node.js + Express 5.x | ES Modules，端口 4000 |
...
| 短信 | SMS Provider 抽象 | mock（开发）/ carrier（生产，CSP V2.4.3） |
...
| [代码审查与优化总结报告.md](代码审查与优化总结报告.md) | 版本迭代记录：128 个问题（v1.0 ~ v7.2.0） |
