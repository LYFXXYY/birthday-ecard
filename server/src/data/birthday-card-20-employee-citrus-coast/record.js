...
const PAGE_DURATION = 6.5; // seconds per page (7 pages = 26s total)
...
function findFFmpeg() {
  // 1. 优先使用 @ffmpeg-installer/ffmpeg（支持 libx264 编码 MP4）
  try {
  ...
  // 3. 回退：Playwright 自带 ffmpeg（仅支持 VP8/WebM，不支持 libx264）
...
async function record() {
  // 支持命令行指定输出路径：node record.js [outputPath]
